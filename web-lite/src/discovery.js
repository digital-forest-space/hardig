 ...
 * PromoConfig layout (345 bytes total):
...
export async function discoverPromos(connection) {
  ...
  try {
    ...
    for (const { pubkey, account } of promoAccounts) {
      ...
      const initialFillBps = view.getUint16(offset, true); // u16 LE
      offset += 2;
